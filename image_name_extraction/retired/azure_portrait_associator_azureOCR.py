...
class BookPortraitAssociator:
    ...
    def load_page_content(self, page_dir: Path) -> Optional[Dict]:
        ...
        try:
            ...
            # Since we're only looking for figure placeholders, no need to scan for actual image files
            page_info = {
                ...
                "md_file": str(md_file),
                "content": content,
                "available_images": []  # Empty since we're only looking for placeholders
            }
...
Portraits/images are referenced as <figure></figure> in the markdown.
...
1. **Exact syntax** – Every portrait appears as <figure></figure>. This is the image teg. Search **only** for this pattern. 
...
3. **Paragraph-embedded rule** – If the image tag <figure></figure> sits inside a prose paragraph that is clearly describing a person, pair the image with that individual rather than the next standalone name heading.  
4. **Page-start rule (cross-page)** – When a markdown page *begins* with an image tag, assume the portrait belongs to the person whose name appeared last on the *previous* page. Mark is_cross_page = true and cross_page_type = "image_first_on_next_page" and associated_person="previous page". The exeption is if the **Immediate-name rule (highest certainty)** applies, in which case use that name instead. then  Mark is_cross_page = false and cross_page_type = "same_page". 
