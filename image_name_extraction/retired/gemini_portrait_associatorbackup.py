...
2. **Labelling the portraits** – All portraits are identified by <figure></figure>. Please enumerate them by their apperance, such that the figure appearing first is labelled Figure 1, the figure appearing second is labelled Figure 2 etc.
...
4. **Page-start rule (cross-page)** – When a markdown page *begins* with an image tag, assume the portrait belongs to the person whose name appeared last on the *previous* page. Mark is_cross_page = true and cross_page_type = "image_first_on_next_page". The exeption is if the **Immediate-name rule (highest certainty)** applies, in which case use that name instead. then  Mark is_cross_page = false and cross_page_type = "same_page". 
