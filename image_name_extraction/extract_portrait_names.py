...
def extract_names_to_dataframe(json_directory):
    ...
    for json_file in json_dir.glob("digibok_2007031501007_*_portrait_associations.json"):
    ...
    try:
        ...
        if len(df) > 0:
            ...
            for i in range(min(3, len(df))):
                print(f"\nName: {df.iloc[i]['name']}")
                print(f"Image: {df.iloc[i]['image_filename']}")
