...
class BookPortraitAssociator:
    ...
    def find_page_directories(self, input_path: Path, single_file: str = None, file_range: Tuple[int, int] = None) -> Dict[str, List[Path]]:
...
3. Portraits/images are tagged as either **[FIGURE: figure.id]**, ![Figure](figures/figure.id) or ![](images/figure.id) in the markdown.
...
1. **Exact syntax** – Every portrait appears as either **[FIGURE: figure.id]**, ![Figure](figures/figure.id) or ![](images/figure.id). Search **only** for this pattern. Here figure.id is the image filename , e.g., 1.1, 1.2, digibok_2007031501007_0057_figure_006.png etc. 
2. **Immediate-name rule (highest certainty)** – If the image tag is followed  by a name whose LASTNAME is in ALL-CAPS, **always pair this image with that name**. Treat this as a 100 % match unless another image intervenes. IMPORTANT: If the name is not in ALL-CAPS, do NOT use it for pairing.
...
4. **Page-start rule (cross-page)** – When a markdown page *begins* with an image tag, assume the portrait belongs to the person whose name appeared last on the *previous* page. Do not attempt to make an association. The exeption is if the **Immediate-name rule (highest certainty)** applies, in which case use that name instead. 
...
# python gemini_portrait_name_associator.py "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_md_and_json" "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_portrait_name_associations" --single-file "digibok_2007031501007_0103.md"

# Process a range of files (pages 100-110):
# python gemini_portrait_name_associator.py "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_md_and_json"  "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_portrait_name_associations" --range 100 110

# Process specific files using comma-separated list:
...
# # Process from 0057 to 0494 (your specific case)
# python gemini_portrait_name_associator.py "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_md_and_json" "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_portrait_name_associations" --start "0057" --end "0494"

# # Process from beginning to 0494
# python gemini_portrait_name_associator.py "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_md_and_json" "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_portrait_name_associations" --end "0494"

# # Process from 0057 to end
# python gemini_portrait_name_associator.py "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_md_and_json" "d:\data\HCNC\norway\biographies\storage\AzureOCR\output_portrait_name_associations" --start "0057"